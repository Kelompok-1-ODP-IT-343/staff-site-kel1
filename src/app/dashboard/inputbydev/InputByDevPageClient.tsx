...
export default function InputByDevPageClient() {
    ...
    <div className="p-6 max-w-[1300px] mx-auto">
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-semibold text-gray-900 dark:!text-white">Input Properti oleh Developer</h1>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="relative w-full h-56 rounded-lg overflow-hidden border">
            <Image src={editedData.image_url} alt={editedData.title} fill className="object-cover" />
          </div>
          ...
          <div className="space-y-2">
            {isEditing ? (
              <>
                <Input value={editedData.title} onChange={(e) => handleChange("title", e.target.value)} className="text-lg font-semibold" />
                <Input value={editedData.description} onChange={(e) => handleChange("description", e.target.value)} className="text-sm" />
              </>
            ) : (
              <>
                <h2 className="font-semibold text-lg text-gray-900 dark:!text-white">{editedData.title}</h2>
          ...
          <div className="rounded-lg border bg-muted/30 p-4 text-sm grid grid-cols-2 gap-y-2">
            {[
              ...
              <div key={key} className="flex justify-between items-center gap-3">
                ...
                {isEditing ? (
                  <Input value={(editedData as any)[key]} onChange={(e) => handleChange(key, e.target.value)} className="h-7 text-sm max-w-[180px]" />
                ) : (
                  <span className="text-right w-[55%] truncate">
                    {String(key).includes("price")
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="rounded-lg border bg-muted/30 p-4 space-y-3">
            <h3 className="font-semibold text/base text-gray-900 dark:!text-white">Property Details</h3>
              ...
              {[
                ...
                return (
                  <div key={key} className="flex justify-between items-center gap-3">
                    ...
                    {isEditing ? (
                      <Input value={value as any} onChange={(e) => handleChange(key, e.target.value)} className="h-7 text-sm max-w-[140px]" />
                    ) : (
                      <span className="text-right truncate max-w-[180px]">{displayValue as any}</span>
                    )}
                  </div>
                );
              })}
          ...
          {/* Location Details */}
          <div className="rounded-lg border bg-muted/30 p-4 space-y-3">
            <h3 className="font-semibold text-base text-gray-900 dark:!text-white">Location Details</h3>
            <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
              {[
                ["Alamat", "address"],
                ...
                <div key={key} className="flex justify-between items-center gap-3">
                  ...
                  {isEditing ? (
                    ...
                    <span className="text-right truncate max-w-[180px]">{(editedData as any)[key]}</span>
                  )}
                </div>
              ))}
            </div>
      ...
      <div className="mt-6 text-xs text-muted-foreground text-center">Data properti dilindungi oleh kebijakan privasi dan peraturan BNI.</div>
